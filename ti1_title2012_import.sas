...
run;
*** Level, Field, Length, Variable, var_idx ***;
...
%macro data_transpose(data_in, data_out, var_id, var_keep=_ALL_, var_idx=);
	...
	*** about %eval func http://goo.gl/eQWm3C ***;
	%let tmpnobs = %eval(0);
	...
	%else %do;
		...
		%do i = 1 %to &tmpmaxidx;
			...
			set tmp_&i;
			%do j = 1 %to &tmpmaxv;
				/* %sysfunc Function: Execute SAS functions or user-written functions.; http://goo.gl/SY0qiy */;
